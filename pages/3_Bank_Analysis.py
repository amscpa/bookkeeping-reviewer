"""
pages/3_Bank_Analysis.py  —  Bank Statement Year-over-Year Analysis
Reads two bookkeeping Excel files, aggregates each account column in Python,
then uses OpenAI o3 to explain WHY each account changed using transaction descriptions.

WORKBOOK LAYOUT (your specific template):
  B2        = Company name
  B4        = Year-end description (year extracted automatically)
  Row 6     = Account short names  (col J onwards, col I = Bank excluded)
  Row 11    = Opening balances
  Rows 12-596 = Transactions  (col C=Date, col D=Description, col E=Comments)
  Row 600   = Closing balances
  Row 603   = IS/BS flags ('IS' or 'BS')
  Col I     = Bank/cash account — EXCLUDED from analysis
  Col J+    = All other accounts to analyse
"""
...
def extract_bank_data(f, sheet_name):
    """
    Returns dict:
      { company, year_label,
        accounts: [{name, acct_type, opening, closing, net_movement, transactions}] }
    """
    f.seek(0)
    ...
    # Account names — row 6, col J onwards
    ...
    for i, name in enumerate(acct_names):
        ...
        for _, r in top.iterrows():
            ...
            if cmt and cmt not in ("nan", "None", ""):
                desc = f"{desc} ({cmt})"
            txns.append({"date": date_s, "desc": desc[:80], "amount": r["_amt"]})

        flag = acct_types[i] if i < len(acct_types) else "IS"
        acct_type = "BS" if flag in ("BS", "B", "BALANCE", "BALANCE SHEET") else "IS"
...
def build_comparison(cy_data, py_data):
    cy_accts = cy_data["accounts"]
    py_accts = py_data["accounts"]
    cy_map   = {a["name"]: a for a in cy_accts}
    py_map   = {a["name"]: a for a in py_accts}
    names    = list(dict.fromkeys([a["name"] for a in cy_accts] +
                                   [a["name"] for a in py_accts]))
    result = []
    for name in names:
        cy = cy_map.get(name)
        py = py_map.get(name)
...
You are reviewing detailed bookkeeping bank statement data comparing two fiscal years.
Each account shows net movement, opening/closing balances, and the largest transactions
with dates and descriptions. Explain WHY each account changed — use transaction descriptions
as evidence. Reference specific dates, amounts, and payee names. All amounts are CAD."""
...
def build_prompt(comparison, cy, py):
    lines = [
        "BANK STATEMENT — YEAR-OVER-YEAR ANALYSIS",
        f"Current Year: {cy}    Prior Year: {py}",
        "Accounts sorted by largest absolute dollar change.",
        "",
    ]
    for a in comparison:
        lines += [
            "=" * 55,
            f"ACCOUNT: {a['name']}  [{a['acct_type']}]",
            f"  Net:  PY {fmt(a['py_net'])}  →  CY {fmt(a['cy_net'])}"
            f"  |  Change: {fmt(a['change_dollar'])}  ({a['change_pct']:+.1f}%)",
            f"  Closing:  PY {fmt(a['py_closing'])}  →  CY {fmt(a['cy_closing'])}",
        ]
        if a["cy_txns"]:
            lines.append(f"  Top {cy} transactions:")
            for t in a["cy_txns"]:
                lines.append(f"    {t['date']}  |  {t['desc']}  |  {fmt(t['amount'])}")
        if a["py_txns"]:
            lines.append(f"  Top {py} transactions:")
            for t in a["py_txns"]:
                lines.append(f"    {t['date']}  |  {t['desc']}  |  {fmt(t['amount'])}")
        lines.append("")
...
## 1. EXECUTIVE SUMMARY
4-5 sentences: overall picture, top 2-3 accounts driving the change, any red flags.

## 2. ACCOUNT-BY-ACCOUNT ANALYSIS
For EVERY account: state dollar/% change, identify specific transactions causing it,
explain if it is a concern, positive trend, or one-time item.

## 3. INCOME STATEMENT ACCOUNTS SUMMARY
Markdown table: | Account | {py} | {cy} | $ Change | % Change | Key Driver |
Income Statement accounts only, sorted by absolute $ change.

## 4. BALANCE SHEET ACCOUNTS SUMMARY
Markdown table: | Account | {py} Closing | {cy} Closing | $ Change | % Change | Key Driver |
Balance Sheet accounts only.

## 5. RED FLAGS & CRA RISK ITEMS
Numbered list. Each: account, concern, risk level (High/Medium/Low), action.
Focus on: unusual descriptions, round-number entries, shareholder payments,
missing expected transactions.

## 6. CLIENT MEETING TALKING POINTS
4-6 specific questions or observations referencing actual transaction descriptions.
""".strip()
...
def flag_rl(flag):
    if "▲" in flag or flag.startswith("+"): return GREEN_BG
    if "▼" in flag or flag.startswith("("): return RED_BG
    if "⚠" in flag:                         return AMBER_BG
    return None


def flag_word(flag):
    if "▲" in flag or flag.startswith("+"): return W_GREEN
    if "▼" in flag or flag.startswith("("): return W_RED
...
def build_pdf(sections, comparison, cy, py, firm, preparer):
    ...
    sty_bul = S("bul", fontSize=9.5, fontName="Helvetica", leading=13,
    ...
    def banner_tbl(text, sty, bg, top_pad=10, bot_pad=10):
    ...
    # Cover
    story.append(banner_tbl(firm.upper(),              sty_wh,  NAVY, 18, 10))
    story.append(banner_tbl("BANK STATEMENT ANALYSIS", sty_wh2, TEAL, 20, 20))
    story.append(banner_tbl(f"{py}  →  {cy}",          sty_wh3, TEAL, 8, 14))
    ...
    story.append(KeepTogether([
        banner_tbl("ACCOUNT VARIANCE QUICK REFERENCE", sty_sec, NAVY, 7, 7),
        Spacer(1, 0.05*inch),
    ]))

    hdr = [Paragraph(h, sty_th) for h in
           ["Account","Type", f"{py} Net", f"{cy} Net", "$ Change", "% Change"]]
    var_rows = [hdr]
    var_style = [
    ...
    for ri, a in enumerate(comparison, start=1):
        ...
        if a["change_dollar"] > 500:
            var_style.append(("BACKGROUND",(0,ri),(-1,ri), GREEN_BG))
        elif a["change_dollar"] < -500:
            var_style.append(("BACKGROUND",(0,ri),(-1,ri), RED_BG))

    vt = Table(var_rows,
        colWidths=[2.4*inch,0.5*inch,1.0*inch,1.0*inch,1.0*inch,1.1*inch],
        repeatRows=1)
    vt.setStyle(TableStyle(var_style))
    story.append(vt)
    story.append(PageBreak())
    ...
    for sec in sections:
        if not sec["body"]:
            continue
        story.append(KeepTogether([
            banner_tbl(sec["title"], sty_sec, NAVY, 7, 7),
            Spacer(1, 0.05*inch),
        ]))
        ...
        if len(trows) >= 2:
            ...
            for ri, r in enumerate(data_rows, start=1):
                ...
                for ci, cell in enumerate(r[:ncols]):
                    s = sty_tdr if 1 <= ci < ncols-1 else sty_td
                    row_cells.append(Paragraph(strip_md(cell), s))
...
def build_word(sections, comparison, cy, py, firm, preparer):
    ...
    _run(ht.cell(0, 0).paragraphs[0],
         "ACCOUNT VARIANCE QUICK REFERENCE",
    ...
    for ci, h in enumerate(["Account","Type",
                              f"{py} Net", f"{cy} Net",
                              "$ Change", "% Change"]):
...
<div class="bank-hero">
  ...
  <p>Python aggregates all 600 transaction rows per account — AI then explains
     WHY each account changed using actual transaction descriptions.</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    api_key = ""
    for key in [["OPENAI_API_KEY"], ["openai", "api_key"]]:
        if api_key:
            break
        try:
            val = st.secrets
    ...
    model = st.selectbox("AI Model",
        ["o3", "o4-mini", "gpt-4o"], index=0, key="bank_model",
        help="o3 = best analysis (30-90s). o4-mini = faster. gpt-4o = quickest.")
...
with col_py:
    ...
    py_label = st.text_input("Year label (or leave blank for auto-detect)",
                              value="", key="bank_py_lbl")
    py_file  = st.file_uploader("Prior Year Excel (.xlsm/.xlsx)",
                                 type=["xlsx", "xls", "xlsm"], key="bank_py_file")

with col_cy:
    st.markdown("#### Current Year")
    cy_label = st.text_input("Year label (or leave blank for auto-detect)",
                              value="", key="bank_cy_lbl")
...
if py_file and cy_file:
    ...
    c1, c2 = st.columns(2)
    with c1:
        py_sheet = st.selectbox("Sheet — Prior Year",    py_sheets, key="bank_py_sheet")
    with c2:
        cy_sheet = st.selectbox("Sheet — Current Year",  cy_sheets, key="bank_cy_sheet")
    ...
    if run_btn:
        if not api_key:
            ...
            prog = st.progress(0, text="Reading prior year…")
            try:
                py_data  = extract_bank_data(py_file, py_sheet)
                prog.progress(25, text="Reading current year…")

                cy_data  = extract_bank_data(cy_file, cy_sheet)
                n        = len(cy_data["accounts"])

                # Use auto-detected year unless user typed something
                py_lbl   = py_label.strip() or py_data["year_label"]
                cy_lbl   = cy_label.strip() or cy_data["year_label"]
                company  = cy_data["company"]

                prog.progress(40, text=f"Found {n} accounts — building comparison…")
                comparison = build_comparison(cy_data, py_data)

                prog.progress(55, text=f"Calling {model} — please wait…")
...
if "bank_raw" in st.session_state:
    ...
    st.success(f"✅  {company}  |  {len(comp)} accounts  |  {py_lbl} vs {cy_lbl}")
    st.divider()
    ...
    with d1:
        st.download_button("📄 Download PDF",
            st.session_state["bank_pdf"],
            f"BankAnalysis_{py_lbl}_vs_{cy_lbl}.pdf",
            "application/pdf", use_container_width=True, key="bank_dl_pdf")
    with d2:
        st.download_button("📝 Download Word",
            st.session_state["bank_word"],
    ...
    with d3:
        st.download_button("📋 Download Text",
            ...
            f"BankAnalysis_{py_lbl}_vs_{cy_lbl}.txt",
            "text/plain", use_container_width=True, key="bank_dl_txt")
    ...
    if st.button("🗑️ Clear Results", key="bank_clear"):
        for k in ["bank_raw","bank_sections","bank_comparison",
                  "bank_pdf","bank_word","bank_cy","bank_py","bank_company"]:
